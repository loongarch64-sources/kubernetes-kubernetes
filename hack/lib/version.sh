...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='af46c47ce925f4c4ad5cc8d1fca46c7b77d13b38'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'tag: v1.20.1-rc.0, tag: v1.20.0' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'tag: v1.20.1-rc.0, tag: v1.20.0' =~ tag:\ (v[^ ,]+) ]]; then
